...
export class FocusTrap {
    constructor(private body: HTMLElement, private previousFocusedElement: HTMLElement) {}

    public restoreFocus() {
        if (this.previousFocusedElement && this.previousFocusedElement.focus && this.body.contains(this.previousFocusedElement)) {
...
export class FocusTrapFactory {
    ...
    private _getFirstFocusableElement(element: HTMLElement): HTMLElement {
        const elements: HTMLElement[] = Array.from(element.querySelectorAll(FOCUSABLE_ELEMENTS_SELECTOR) as NodeListOf<HTMLElement>).filter(
