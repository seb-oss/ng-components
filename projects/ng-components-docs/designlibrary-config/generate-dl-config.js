...
const generateDLFiles = async function() {
  ...
  const exampleModuleFiles = await globby(
    path.join(__dirname, '../src/app/examples/components/**/*.module.ts')
  );
  ...
  for (let i = 0; i < components.length; i++) {
    ...
    const frontMatter = htmlFrontMatter(
      fs.readFileSync(
        path.join(exampleFilesBasePath, component.htmlFile),
        'utf-8'
      )
    );
    const exampleFileMetadata = {
      ...frontMatter,
      ...component,
      iframesource: frontMatter.url + '/' + component.route,
    };
    ...
    const targetDir = path.join(
      __dirname,
      '../../../dist/ng-components-docs/design-library'
    );
    fs.mkdirSync(targetDir, { recursive: true });
    fs.writeFileSync(
      path.join(targetDir, frontMatter.componentid + '.md'),
      renderedMarkdown
    );
  }
