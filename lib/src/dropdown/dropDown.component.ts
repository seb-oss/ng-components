...
export class DropDownComponent
  ...
  @Input() handleChange?: (
    event: DropdownItem | Array<DropdownItem> | UIEvent
  ) => void;
  ...
  @ViewChild('dropdownToggleRef', { read: ElementRef, static: true })
  dropdownToggleRef: ElementRef;
  @ViewChild('dropdownMenuRef', { read: ElementRef, static: true })
  dropdownMenuRef: ElementRef;
  @ViewChild('searchRef', { read: ElementRef, static: true })
  ...
  private _generateHelperArrays(caller?: string) {
    ...
    this.uniqueList =
      this.list &&
      this.list
        .filter(e => e && e.hasOwnProperty('value') && e.hasOwnProperty('text'))
        .map((e, i) => {
          const id = `${e.value}-${i}`;
          let selected = false;
          ...
          if (!this.multi) {
            if (
              (this.selectedValue as DropdownItem) &&
              e.value === (this.selectedValue as DropdownItem).value
            ) {
              selected = true;
            }
          } else {
            if (
              ...
              (this.selectedValue as Array<DropdownItem>).find(
                el => el.value === e.value
              )
            ) {
              selected = true;
    ...
    this.selectedList =
      this.uniqueList &&
      this.uniqueList.filter(e => e.selected).map(e => e.dropdownItem);
    this.allSelected =
      this.selectedList && this.uniqueList
        ? this.selectedList.length === this.uniqueList.length
        : false;
      ...
      this.displayList = [
        {
          ...
          className: `dropdown-item select-all custom-dropdown-item multi${
      ...
      switch (key) {
        ...
        case 'enter':
          ...
          if (
            ...
            (this.searchText.length === 0 && this.currentFocused > 0) ||
            this.searchText.length > 0
          ) {
            this.dropdownItemSelected(
              this.displayList[this.currentFocused].dropdownItem
            );
          }
          break;
        case 'arrowdown':
  ...
  dropdownItemSelected(item: DropdownItem): void {
    if (!this.multi) {
      ...
      const currentList = (this.selectedValue as Array<DropdownItem>)
        ? (this.selectedValue as Array<DropdownItem>)
        : [];
