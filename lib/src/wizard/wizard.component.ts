...
export class SebWizardComponent implements AfterContentInit, OnDestroy {
  ...
  @ContentChildren(SebWizardStepComponent) wizardSteps: QueryList<
    SebWizardStepComponent
  >;

  @Output() onChanges: EventEmitter<ISebWizardChangeEvent> = new EventEmitter<
    ISebWizardChangeEvent
  >();
  ...
  private _validStepControl(index: number): boolean {
    ...
    if (this.stepWise) {
      return !wizardSteps
        .slice(0, index)
        .some((wizardStep: SebWizardStepComponent) => {
